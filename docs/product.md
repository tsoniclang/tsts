...
counts. TSTS exact-joins every count before installing output. The current
pinned product accepts exactly 72 complete canonical pointer-key map rewrites,
3,429 static-key property-location constructions, and 97 emitted
property-location classes. A source, compiler, or target-pin change that
produces any other denominator must be re-reviewed and recertified rather than
silently widening or shrinking an optimized class.
