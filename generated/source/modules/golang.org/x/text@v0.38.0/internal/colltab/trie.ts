...
export class Trie {
    ...
    static $go$private$colltab$lookup(t: Trie | undefined, s: RuntimeSlice<uint8>): [
        Elem,
        int
    ] {
        let v: Elem = 0;
        ...
        __gotots_control_target_0: {
            if (c0 < tx$uint8) {
                return [(t ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).Values0.get(c0), 1];
            }
            else if (c0 < t2$uint8) {
                return [0, 1];
            }
                ...
                let i = (t ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).Index0.get(c0);
                let c1 = s.get(1);
                ...
                let o = (i << 6) + c1;
                i = (t ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).Index.get(o);
                let c2 = s.get(2);
    ...
    static $go$private$colltab$lookupString(t: Trie | undefined, s: gostring): [
        Elem,
        int
    ] {
        let v: Elem = 0;
    ...
    static $go$private$colltab$lookupValue(t: Trie | undefined, n: uint16, b: uint8): Elem {
        return (t ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).Values.get((n << 6) + b);
    }
}
