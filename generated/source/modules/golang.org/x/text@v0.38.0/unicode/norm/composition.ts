...
export class reorderBuffer {
    declare private readonly $goType: void;
    public constructor(public rune: GoArray<Properties__from_norm$Storage, 32>, public byte: GoArray<uint8, 128>, public nbyte: uint8, public ss: streamSafe, public nrune: int, public f: formInfo, public src: input, public nsrc: int, public tmpBytes: input, public __go_out: RuntimeSlice<uint8>, public flushF: (($0: reorderBuffer | undefined) => bool) | undefined) {
    }
    declare private readonly then?: never;
    static $go$private$norm$appendRune(rb: reorderBuffer | undefined, r: int32): void {
        let bn = (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nbyte;
        let sz = globalThis.Number(BigInt.asIntN(64, utf8__from_gostdlib.EncodeRune(goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, bn, null, null), r)));
        ...
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.set((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune, (void Properties.$storageOf, (void Properties.$fromStorage,
            {
                pos: bn,
                size: sz & 255,
                ccc: 0,
    ...
    static $go$private$norm$assignRune(rb: reorderBuffer | undefined, pos: int, r: int32): void {
        let bn = (void Properties.$storageOf, (void Properties.$fromStorage,
            (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.get(pos))).pos;
        let sz = globalThis.Number(BigInt.asIntN(64, utf8__from_gostdlib.EncodeRune(goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, bn, null, null), r)));
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.set(pos, (void Properties.$storageOf, (void Properties.$fromStorage,
            {
                pos: bn,
                size: sz & 255,
                ccc: 0,
    ...
    static $go$private$norm$bytesAt(rb: reorderBuffer | undefined, n: int): RuntimeSlice<uint8> {
        let inf = Properties.$copy(Properties.$fromStorage((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.get(n)));
        return goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, Properties.$storageOf(inf).pos, Properties.$storageOf(inf).pos + Properties.$storageOf(inf).size, null);
    }
    static $go$private$norm$combineHangul(rb: reorderBuffer | undefined, s: int, i: int, k: int): void {
        let b = goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune, 0, null, null);
        let bn = (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune;
        for (; i < bn; i++) {
        ...
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune = k;
    }
    static $go$private$norm$compose(rb: reorderBuffer | undefined): void {
    ...
    static $go$private$norm$decomposeHangul(rb: reorderBuffer | undefined, r: int32): void {
    ...
    static $go$private$norm$doFlush(rb: reorderBuffer | undefined): bool {
        if ((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).f.composing) {
            reorderBuffer.$go$private$norm$compose(rb);
        }
        const __gotots_callee_0 = (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).flushF;
    ...
    static $go$private$norm$flushCopy(rb: reorderBuffer | undefined, buf: RuntimeSlice<uint8>): int {
        let p = 0;
        for (let i = 0; i < (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune; i++) {
            let runep = Properties.$copy(Properties.$fromStorage((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.get(i)));
            p += RuntimeSlice.copy<uint8>(buf.slice(p, null, null), goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, Properties.$storageOf(runep).pos, Properties.$storageOf(runep).pos + Properties.$storageOf(runep).size, null));
        }
        reorderBuffer.$go$private$norm$reset(rb);
        return p;
    }
    static $go$private$norm$insertCGJ(rb: reorderBuffer | undefined): void {
    ...
    static $go$private$norm$insertDecomposed(rb: reorderBuffer | undefined, dcomp: RuntimeSlice<uint8>): insertErr {
        const __gotots_store_1 = (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference"));
        input.$go$private$norm$setBytes(__gotots_store_1.tmpBytes, dcomp);
        for (let i = 0; i < dcomp.length;) {
            ...
            const __gotots_argument_3 = input.$copy((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).tmpBytes);
            ...
            if (info.BoundaryBefore() && (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune > 0 && !reorderBuffer.$go$private$norm$doFlush(rb)) {
                return iShortDst$constant();
            }
            i += RuntimeSlice.copy<uint8>(goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nbyte, null, null), dcomp.slice(i, i + Properties.$storageOf(info).size, null));
            reorderBuffer.$go$private$norm$insertOrdered(rb, Properties.$copy(info));
        }
        return iSuccess$constant();
    }
    static $go$private$norm$insertFlush(rb: reorderBuffer | undefined, src: input, i: int, info: Properties): insertErr {
        {
            let rune = input.$go$private$norm$hangul(src, i);
            if (rune !== 0) {
    ...
    static $go$private$norm$insertOrdered(rb: reorderBuffer | undefined, info: Properties): void {
        let n = (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune;
        let b = goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune, 0, null, null);
        ...
        let pos = (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nbyte;
    ...
    static $go$private$norm$insertSingle(rb: reorderBuffer | undefined, src: input, i: int, info: Properties): void {
        input.$go$private$norm$copySlice(src, goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nbyte, null, null), i, i + Properties.$storageOf(info).size);
        reorderBuffer.$go$private$norm$insertOrdered(rb, Properties.$copy(info));
    }
    static $go$private$norm$insertUnsafe(rb: reorderBuffer | undefined, src: input, i: int, info: Properties): void {
        {
            let rune = input.$go$private$norm$hangul(src, i);
            if (rune !== 0) {
                reorderBuffer.$go$private$norm$decomposeHangul(rb, rune);
    ...
    static $go$private$norm$reset(rb: reorderBuffer | undefined): void {
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune = 0;
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nbyte = 0;
    }
    static $go$private$norm$runeAt(rb: reorderBuffer | undefined, n: int): int32 {
        let inf = Properties.$copy(Properties.$fromStorage((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.get(n)));
        const __gotots_results_0 = utf8__from_gostdlib.DecodeRune(goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, Properties.$storageOf(inf).pos, Properties.$storageOf(inf).pos + Properties.$storageOf(inf).size, null));
    ...
    static $go$private$norm$setFlusher(rb: reorderBuffer | undefined, __go_out: RuntimeSlice<uint8>, f: (($0: reorderBuffer | undefined) => bool) | undefined): void {
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).__go_out = __go_out;
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).flushF = f;
    }
}
export function appendFlush(rb: reorderBuffer | undefined): bool {
    for (let i = 0; i < (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).nrune; i++) {
        let start = (void Properties.$storageOf, (void Properties.$fromStorage,
            (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.get(i))).pos;
        let end = start + (void Properties.$storageOf, (void Properties.$fromStorage,
            (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).rune.get(i))).size;
        (rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).__go_out = goSliceAppendSlice<uint8>((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).__go_out, goArraySlice((rb ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).byte, start, end, null), 0);
    }
    return true;
}
