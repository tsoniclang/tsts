...
export class NodeFactory {
    ...
    static CreateExpressionFromEntityName(f: {
        ...
        if (IsQualifiedName__from_ast(node)) {
            ...
            const __gotots_receiver_5 = (Node__from_ast.AsQualifiedName(node) ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).value.Right;
            const __gotots_store_6 = (f ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).value;
            const __gotots_argument_11 = new $goInterfaceAdapter$PointerTo_Named_ast$NodeFactory(NodeFactory__from_ast.AsNodeFactory(tsonicTypeScriptRuntime.propertyLocation(__gotots_store_6, "NodeFactory")));
        ...
        const __gotots_argument_12 = new $goInterfaceAdapter$PointerTo_Named_ast$NodeFactory(NodeFactory__from_ast.AsNodeFactory(tsonicTypeScriptRuntime.propertyLocation(__gotots_store_8, "NodeFactory")));
    ...
    static CreateForOfBindingStatement(f: {
        ...
        if (IsVariableDeclarationList__from_ast(node)) {
            ...
            const __gotots_argument_170 = Node__from_ast.$storageOf(Node__from_ast.$fromStorage(NodeDefault__from_ast.$storageOf(NodeDefault__from_ast.$fromStorage(NodeBase__from_ast.$storageOf(NodeBase__from_ast.$fromStorage(VariableDeclarationList__from_ast.$storageOf(((Node__from_ast.AsVariableDeclarationList(node) ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<VariableDeclarationList__from_ast>).value).NodeBase)).NodeDefault)).Node)).Flags;
