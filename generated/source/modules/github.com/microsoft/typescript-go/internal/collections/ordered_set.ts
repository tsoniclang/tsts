...
export class OrderedSet<T> {
    ...
    static Add$kernel<T>(s: tsonicTypeScriptRuntime.Location<OrderedSet<T>> | undefined, $go$copy$T0_to_T0: ($0: T) => T, $go$from_container_storage$T0_to_T0: ($0: GoContainerStorage<T>) => T, $go$map_construct$Struct_void_to_MapOf_T0_To_Struct_void: ($0: GoEmptyStruct) => GoMapValue<T, GoEmptyStruct>, $go$to_container_storage$T0_to_T0: ($0: T) => GoContainerStorage<T>, $go$zero$void_to_T0: () => T, value: T): void {
        ...
        OrderedMap.Set$kernel<T, GoEmptyStruct>(tsonicTypeScriptRuntime.projectLocation<OrderedMap__from_collections$Storage<T, GoEmptyStruct>, OrderedMap<T, GoEmptyStruct>>(tsonicTypeScriptRuntime.propertyLocation(__gotots_store_1, "m"), ($go$storage: OrderedMap__from_collections$Storage<T, GoEmptyStruct>): OrderedMap<T, GoEmptyStruct> => {
            return OrderedMap.$fromStorage<T, GoEmptyStruct>($go$storage);
        }, ($go$value: OrderedMap<T, GoEmptyStruct>): OrderedMap__from_collections$Storage<T, GoEmptyStruct> => {
            return OrderedMap.$storageOf<T, GoEmptyStruct>($go$value);
        }), $go$copy$T0_to_T0, $go$from_container_storage$T0_to_T0, $go$map_construct$Struct_void_to_MapOf_T0_To_Struct_void, $go$to_container_storage$T0_to_T0, $go$zero$void_to_T0, (): GoEmptyStruct => {
    ...
    static Size$kernel<T>(s: tsonicTypeScriptRuntime.Location<OrderedSet<T>> | undefined, $go$length$SliceOf_T0_to_int: ($0: RuntimeSlice<GoContainerStorage<T>>) => int): int {
        ...
        return OrderedMap.Size$kernel<T, GoEmptyStruct>(tsonicTypeScriptRuntime.projectLocation<OrderedMap__from_collections$Storage<T, GoEmptyStruct>, OrderedMap<T, GoEmptyStruct>>(tsonicTypeScriptRuntime.propertyLocation(__gotots_store_0, "m"), ($go$storage: OrderedMap__from_collections$Storage<T, GoEmptyStruct>): OrderedMap<T, GoEmptyStruct> => {
            return OrderedMap.$fromStorage<T, GoEmptyStruct>($go$storage);
        }, ($go$value: OrderedMap<T, GoEmptyStruct>): OrderedMap__from_collections$Storage<T, GoEmptyStruct> => {
            return OrderedMap.$storageOf<T, GoEmptyStruct>($go$value);
        }), $go$length$SliceOf_T0_to_int);
    }
