...
export class importAdder {
    ...
    public constructor(public ctx: GoInterface | undefined, public checker: tsonicTypeScriptRuntime.Location<Checker__from_checker> | undefined, public view: View | undefined, public formatOptions: FormatCodeSettings__from_lsutil, public converters: {
        ...
        value: Fix;
    } | undefined>, public addToExisting: GoMapValue<tsonicTypeScriptRuntime.Location<Node__from_ast> | undefined, addToExistingState | undefined>, public newImports: GoMapValue<gostring, importsCollection | undefined>) {
    }
    declare private readonly then?: never;
            ...
            case AutoImportFixKindAddNew$constant__from_lsproto(): {
                ...
                switch (((fix ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).value.AutoImportFix ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).value.ImportKind) {
                    case ImportKindDefault$constant__from_lsproto(): {
                        Assert__from_debug((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).defaultImport === undefined || (((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).defaultImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<newImportBinding>).value.name === symbolName, RuntimeSlice.literal<$goInterface$Interface_void | undefined>([new $goInterfaceAdapter$string("(Add new) Default import should be missing or match symbolName")]));
                        let prevTypeOnly = 0;
                        if (!((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).defaultImport === undefined)) {
                            prevTypeOnly = (((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).defaultImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<newImportBinding>).value.addAsTypeOnly;
                        }
                    ...
                    case ImportKindNamed$constant__from_lsproto(): {
                        if ((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namedImports.isNil()) {
                            (entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namedImports = $goMap$MapOf_string_To_PointerTo_Named_autoimport$newImportBinding.make(0, []);
                        }
                        let prevImport: tsonicTypeScriptRuntime.Location<newImportBinding> | undefined = (entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namedImports.lookup(symbolName);
                        let prevTypeOnly = 0;
                        if (!(prevImport === undefined)) {
                    ...
                    case ImportKindCommonJS$constant__from_lsproto(): {
                        if ((compilerOptions ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).value.VerbatimModuleSyntax === TSTrue$constant__from_core()) {
                            ...
                            Assert__from_debug((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namespaceLikeImport === undefined || (((entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namespaceLikeImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<newImportBinding>).value.name === symbolName, RuntimeSlice.literal<$goInterface$Interface_void | undefined>([new $goInterfaceAdapter$string("Namespacelike import should be missing or match symbolName")]));
                            (entry ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namespaceLikeImport =
    ...
    static Edits(adder: importAdder | undefined): RuntimeSlice<{
        ...
        for (const __gotots_range_value_7 of __gotots_range_keys_1) {
            ...
            let newImport: importsCollection | undefined = __gotots_range_value_10;
            ...
            if ((newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).useRequire) {
                declarations = getNewRequires(tracker, moduleSpecifier, quotePreference, (newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).defaultImport, sortedNamedImports((newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namedImports), (newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namespaceLikeImport, Program__from_compiler.Options(((adder ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).view ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).program));
            }
            else {
                declarations = getNewImports(tracker, moduleSpecifier, quotePreference, (newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).defaultImport, sortedNamedImports((newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namedImports), (newImport ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).namespaceLikeImport, Program__from_compiler.Options(((adder ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).view ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).program), UserPreferences__from_lsutil.$copy((adder ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).preferences));
            }
    ...
    static $go$private$autoimport$getNewImportEntry(adder: importAdder | undefined, moduleSpecifier: gostring, importKind: ImportKind__from_lsproto, useRequire: bool, addAsTypeOnly: AddAsTypeOnly__from_lsproto): importsCollection | undefined {
        let typeOnlyKey = newImportsKey(moduleSpecifier, true);
        let nonTypeOnlyKey = newImportsKey(moduleSpecifier, false);
        let typeOnlyEntry: importsCollection | undefined = (adder ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).newImports.lookup(typeOnlyKey);
        let nonTypeOnlyEntry: importsCollection | undefined = (adder ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")).newImports.lookup(nonTypeOnlyKey);
        let newEntry: importsCollection | undefined = new importsCollection(void 0, $goMap$MapOf_string_To_PointerTo_Named_autoimport$newImportBinding.nil(), void 0, useRequire);
