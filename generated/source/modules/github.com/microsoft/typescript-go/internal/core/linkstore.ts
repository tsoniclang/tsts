...
export class LinkStore<K, V> {
    ...
    static Get$kernel<K, V>(s: tsonicTypeScriptRuntime.Location<LinkStore<K, V>> | undefined, $go$capacity$SliceOf_T1_to_int: ($0: RuntimeSlice<GoContainerStorage<V>>) => int, $go$convert$SliceOf_T1_to_SliceOf_T1: ($0: RuntimeSlice<GoContainerStorage<V>>) => RuntimeSlice<GoContainerStorage<V>>, $go$copy$T1_to_T1: ($0: V) => V, $go$from_container_storage$T1_to_T1: ($0: GoContainerStorage<V>) => V, $go$index_address$SliceOf_T1_int_to_PointerTo_T1: ($0: RuntimeSlice<GoContainerStorage<V>>, $1: int) => tsonicTypeScriptRuntime.Location<V> | undefined, $go$length$SliceOf_T1_to_int: ($0: RuntimeSlice<GoContainerStorage<V>>) => int, $go$map_construct$PointerTo_T1_to_MapOf_T0_To_PointerTo_T1: ($0: tsonicTypeScriptRuntime.Location<V> | undefined) => GoMapValue<K, tsonicTypeScriptRuntime.Location<V> | undefined>, $go$to_container_storage$T1_to_T1: ($0: V) => GoContainerStorage<V>, $go$zero$void_to_T1: () => V, key__shadow_1: K): tsonicTypeScriptRuntime.Location<V> | undefined {
        let value: tsonicTypeScriptRuntime.Location<V> | undefined = LinkStore.$storageOf(((s ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<LinkStore<K, V>>).value).entries.lookup(key__shadow_1);
        if (!(value === undefined)) {
            return value;
        }
        if (LinkStore.$storageOf(((s ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<LinkStore<K, V>>).value).entries.isNil()) {
            LinkStore.$storageOf(((s ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<LinkStore<K, V>>).value).entries = $go$map_construct$PointerTo_T1_to_MapOf_T0_To_PointerTo_T1(void 0);
        }
        const __gotots_store_0 = LinkStore.$storageOf(((s ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<LinkStore<K, V>>).value);
        value = Arena.New$kernel<V>(tsonicTypeScriptRuntime.projectLocation<Arena__from_core$Storage<V>, Arena<V>>(tsonicTypeScriptRuntime.propertyLocation(__gotots_store_0, "arena"), ($go$storage: Arena__from_core$Storage<V>): Arena<V> => {
            return Arena.$fromStorage<V>($go$storage);
        }, ($go$value: Arena<V>): Arena__from_core$Storage<V> => {
            return Arena.$storageOf<V>($go$value);
        }), $go$capacity$SliceOf_T1_to_int, $go$convert$SliceOf_T1_to_SliceOf_T1, $go$copy$T1_to_T1, $go$from_container_storage$T1_to_T1, $go$index_address$SliceOf_T1_int_to_PointerTo_T1, $go$length$SliceOf_T1_to_int, $go$to_container_storage$T1_to_T1, $go$zero$void_to_T1);
        LinkStore.$storageOf(((s ?? GoPanic.raiseRuntime("invalid memory address or nil pointer dereference")) as tsonicTypeScriptRuntime.Location<LinkStore<K, V>>).value).entries.store(key__shadow_1, value);
        return value;
    }
