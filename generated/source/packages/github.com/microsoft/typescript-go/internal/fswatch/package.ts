...
import { $goInterfaceMethod$Available$void_to_bool, $goInterfaceMethod$Close$void_to_Named_error, $goInterfaceMethod$HasFastRecursiveBackend$void_to_bool, $goInterfaceMethod$Name$void_to_string, $goInterfaceMethod$WatchDirectory$string_Named_fswatch$WatchCallback_Variadic_SliceOf_Named_fswatch$WatchOption_to_Named_fswatch$Watch_Named_error, $goInterfaceMethod$WatchFile$string_Named_fswatch$WatchCallback_to_Named_fswatch$Watch_Named_error, $goInterfaceMethod$fswatch$applyWatchOption$PointerTo_Named_fswatch$watchOptions_to_void, $goInterfaceMethod$fswatch$unexported$void_to_void, } from "../../../../../../support/interface-methods.js";
...
interface NodeFS {
    ...
    }, listener: (event: "change" | "rename", fileName: string | null) => void): NodeFSWatcher;
}
interface NodePath {
...
export function WatchOption$is(value: GoInterfaceValue | undefined): value is WatchOption {
...
export function WithIgnore(fn: ((path: gostring) => bool) | undefined): WatchOption | undefined {
...
export class WatchCallback {
    constructor(public readonly $value: ((events: RuntimeSlice<Event$Storage>, failure: GoInterface | undefined) => void) | undefined) { }
}
...
class WatchHandle extends ProductInterfaceValue implements Watch {
    ...
    constructor(private readonly watcher: NodeFSWatcher, private readonly timer: {
...
export interface Watcher extends GoInterfaceValue {
    ...
    Name(): gostring;
    WatchDirectory(dir: gostring, fn: WatchCallback, options: RuntimeSlice<WatchOption | undefined>): [
        Watch | undefined,
        GoInterface | undefined
    ];
    WatchFile(path: gostring, fn: WatchCallback): [
        Watch | undefined,
        GoInterface | undefined
    ];
    $go$private$fswatch$unexported(): void;
...
class NodeWatcher extends ProductInterfaceValue implements Watcher {
    ...
    constructor(private readonly name: gostring, private readonly available: bool, private readonly fastRecursive: bool) {
        super();
    }
    Available(): bool {
    ...
    WatchDirectory(dir: gostring, fn: WatchCallback, selected: RuntimeSlice<WatchOption | undefined>): [
        Watch | undefined,
        GoInterface | undefined
    ] {
        if (!this.available) {
    ...
    $go$private$fswatch$unexported(): void { }
    private watchPath(root: string, selectedName: string | undefined, fn: WatchCallback, options: WatchOptions): [
        Watch | undefined,
        GoInterface | undefined
    ] {
        ...
        const flush = (): void => {
            ...
            const events = [...pending.entries()].map(([path, kind]) => Event.$storageOf(Event.$make(kind, path)));
            pending.clear();
            fn.$value(RuntimeSlice.literal<Event$Storage>(events), undefined);
        };
        let watcher: NodeFSWatcher;
        try {
            watcher = nodeFS.watch(root, { encoding: "utf8", recursive: options.recursive }, (event, fileName) => {
        ...
        watcher.on("error", (failure) => {
            fn.$value?.(RuntimeSlice.literal<Event$Storage>([]), newError(`fswatch: watch terminated: ${failure.message}`));
        });
