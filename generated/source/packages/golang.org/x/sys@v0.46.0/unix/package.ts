...
import type { gostring, int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64, uintptr, } from "@gotots/runtime/scalars.js";
...
function providerFailure(failure: import("@gotots/runtime/interface-value.js").GoError | undefined): GoFailure {
...
class FanotifyEventMetadata {
    ...
    static $make(eventLength: uint32, version: uint8, reserved: uint8, metadataLength: uint16, mask: uint64, fd: int32, pid: int32): FanotifyEventMetadata {
...
export class Stat_t {
    ...
    static $make(dev: uint64, ino: uint64, nlink: uint64, mode: uint32, uid: uint32, gid: uint32, rdev: uint64, size: int64, blockSize: int64, blocks: int64, accessTime: Timespec, modificationTime: Timespec, changeTime: Timespec): Stat_t {
        return new Stat_t({
            Dev: dev,
            Ino: ino,
            Nlink: nlink,
...
class Dirent {
    ...
    static $make(ino: uint64, offset: int64, recordLength: uint16, type: uint8, name: GoArray<int8, 256>): Dirent {
...
export class Termios {
    ...
    static $make(inputFlags: uint32, outputFlags: uint32, controlFlags: uint32, localFlags: uint32, line: uint8, controlCharacters: GoArray<uint8, 19>, inputSpeed: uint32, outputSpeed: uint32): Termios {
        return new Termios({
            Iflag: inputFlags,
            Oflag: outputFlags,
            Cflag: controlFlags,
...
class Statfs_t {
    ...
    static $make(type: int64, blockSize: int64, blocks: uint64, freeBlocks: uint64, availableBlocks: uint64, files: uint64, freeFiles: uint64, fsid: Fsid, nameLength: int64, fragmentSize: int64, flags: int64, spare: GoArray<int64, 4>): Statfs_t {
        return new Statfs_t({
            Type: type,
            Bsize: blockSize,
            Blocks: blocks,
...
class FileHandle {
    private constructor(private readonly handleType: int32, private readonly bytes: RuntimeSlice<uint8>) { }
    ...
    static Bytes(handle: tsonicTypeScriptRuntime.Location<FileHandle> | undefined): RuntimeSlice<uint8> {
        return handle === undefined ? RuntimeSlice.nil<uint8>() : handle.value.bytes;
    }
    static Size(handle: tsonicTypeScriptRuntime.Location<FileHandle> | undefined): int {
        return FileHandle.Bytes(handle).length;
    }
    static Type(handle: tsonicTypeScriptRuntime.Location<FileHandle> | undefined): int32 {
        return handle === undefined ? 0 : handle.value.handleType;
    }
}
...
const FAN_ONDIR$uint64: uint64 = 1073741824n;
const FAN_Q_OVERFLOW$uint64: uint64 = 16384n;
const FAN_RENAME$uint64: uint64 = 268435456n;
const IN_ISDIR$uint32: uint32 = 1073741824;
const IN_MODIFY$uint32: uint32 = 2;
const IN_Q_OVERFLOW$uint32: uint32 = 16384;
const S_IFDIR$uint32: uint32 = 16384;
const S_IFMT$uint32: uint32 = 61440;
const O_LARGEFILE$int: int = 0;
const SOL_SOCKET$int: int = 1;
const SO_PROTOCOL$int: int = 38;
export const TIOCGWINSZ$uint: uint = 21523;
...
const $state = {};
function ByteSliceFromString(value: gostring): [
    RuntimeSlice<uint8>,
    GoFailure
] {
    if (value.includes("\0")) {
...
function BytePtrFromString(value: gostring): [
    tsonicTypeScriptRuntime.Location<uint8> | undefined,
    GoFailure
] {
...
export function Open(path: gostring, _mode: int, _permissions: uint32): [
    int,
    GoFailure
] {
...
function Openat(directoryDescriptor: int, path: gostring, flags: int, permissions: uint32): [
    int,
    GoFailure
] {
...
function Read(descriptor: int, buffer: RuntimeSlice<uint8>): [
    int,
    GoFailure
] {
    const file = openFiles.get(descriptor);
...
function Write(descriptor: int, buffer: RuntimeSlice<uint8>): [
    int,
    GoFailure
] {
    const file = openFiles.get(descriptor);
...
export function Readlink(path: gostring, buffer: RuntimeSlice<uint8>): [
    int,
    GoFailure
] {
...
function Readlinkat(directoryDescriptor: int, path: gostring, buffer: RuntimeSlice<uint8>): [
    int,
    GoFailure
] {
...
function fillStat(information: fs.FileInfo, target: Stat_t | undefined): GoFailure {
    if (target === undefined) {
        return errnoFailure(14n);
    }
    const stat = target;
    stat.Mode = information.IsDir() ? S_IFDIR$uint32 : 32768;
    stat.Size = information.Size();
    stat.Blksize = 4096n;
    stat.Blocks = (stat.Size + 511n) / 512n;
    return undefined;
}
export function Stat(path: gostring, target: Stat_t | undefined): GoFailure {
    const [information, failure] = os.Stat(path);
    return information === undefined ? providerFailure(failure) : fillStat(information, target);
}
function Lstat(path: gostring, target: Stat_t | undefined): GoFailure {
    const [information, failure] = os.Lstat(path);
    return information === undefined ? providerFailure(failure) : fillStat(information, target);
}
function Fstatat(directoryDescriptor: int, path: gostring, target: Stat_t | undefined, flags: int): GoFailure {
    if (directoryDescriptor !== AT_FDCWD$int) {
        return unsupported();
    }
    return (flags & AT_SYMLINK_NOFOLLOW$int) !== 0 ? Lstat(path, target) : Stat(path, target);
}
function Statfs(path: gostring, target: tsonicTypeScriptRuntime.Location<Statfs_t> | undefined): GoFailure {
    const [information, failure] = os.Stat(path);
...
export function IoctlGetWinsize(_descriptor: int, _request: uint): [
    Winsize | undefined,
    GoFailure
] {
    return [Winsize.$zero(), errnoFailure(25n)];
}
export function IoctlGetTermios(_descriptor: int, _request: uint): [
    Termios | undefined,
    GoFailure
] {
    return [Termios.$zero(), errnoFailure(25n)];
}
function ReadDirent(_descriptor: int, _buffer: RuntimeSlice<uint8>): [
    int,
    GoFailure
] {
    return [0, unsupported()];
}
function Getdents(_descriptor: int, _buffer: RuntimeSlice<uint8>): [
    int,
    GoFailure
] {
    return [0, unsupported()];
...
function Ppoll(_descriptors: RuntimeSlice<PollFd$Storage>, _timeout: tsonicTypeScriptRuntime.Location<Timespec> | undefined, _signalMask: tsonicTypeScriptRuntime.Location<Sigset_t> | undefined): [
    int,
    GoFailure
] {
    return [0, unsupported()];
}
function Poll(descriptors: RuntimeSlice<PollFd$Storage>, _timeout: int): [
    int,
    GoFailure
] {
...
function FanotifyInit(_flags: uint, _eventFlags: uint): [
    int,
    GoFailure
] {
    return [-1, unsupported()];
}
function FanotifyMark(_descriptor: int, _flags: uint, _mask: uint64, _directoryDescriptor: int, _path: gostring): GoFailure {
    return unsupported();
}
function InotifyInit1(_flags: int): [
    int,
    GoFailure
] {
    return [-1, unsupported()];
}
function InotifyAddWatch(_descriptor: int, _path: gostring, _mask: uint32): [
    int,
    GoFailure
] {
    return [-1, unsupported()];
}
function InotifyRmWatch(_descriptor: int, _watchDescriptor: uint32): [
    int,
    GoFailure
] {
    return [-1, unsupported()];
}
function NameToHandleAt(_directoryDescriptor: int, _path: gostring, _flags: int): [
    FileHandle,
    int,
    GoFailure
] {
...
function GetsockoptInt(_descriptor: int, _level: int, _option: int): [
    int,
    GoFailure
] {
    return [0, unsupported()];
}
function Syscall(_trap: uintptr, _argument1: uintptr, _argument2: uintptr, _argument3: uintptr): [
    uintptr,
    uintptr,
    syscall.Errno
] {
...
function Syscall6(_trap: uintptr, _argument1: uintptr, _argument2: uintptr, _argument3: uintptr, _argument4: uintptr, _argument5: uintptr, _argument6: uintptr): [
    uintptr,
    uintptr,
    syscall.Errno
] {
...
function RawSyscall(trap: uintptr, argument1: uintptr, argument2: uintptr, argument3: uintptr): [
    uintptr,
    uintptr,
    syscall.Errno
] {
