...
export function buildIndexedModuleValueEnvironments(api, index) {
  ...
  const resolveDefinition = (id, trail = [], requirePublic = false) => {
    ...
    if (definition.state === "resolved") return definition.value;
    ...
    try {
      ...
      definition.value = definition.implicit === undefined
  ...
  const resolveLocalReference = (moduleId, scope, reference, trail) => {
    ...
    if (named !== undefined && !named.typeOnly) {
      ...
      return resolveExportedConstant(target, trail)?.value;
    }
    const namespace = module.structure.imports.namespaces.get(head);
    if (namespace !== undefined && !namespace.typeOnly && tail.length > 0) {
      return resolveExportedConstant(`${resolveModuleId(namespace.module, moduleId)}::${tail.join(".")}`, trail)?.value;
    }
    return undefined;
  };
  ...
  const resolveExportedConstant = (id, trail = []) => {
    ...
    const value = resolveDefinition(targetId, [...trail, id], tail.length > 0);
    if (value === undefined) return undefined;
    const result = { id: targetId, value };
...
function evaluateImplicitEnumValue(definition, resolveDefinition, trail) {
  if (definition.implicit.previous === undefined) return Object.freeze({ kind: "number", value: 0 });
  const previous = resolveDefinition(definition.implicit.previous, trail);
  if (previous?.kind !== "number") return undefined;
  return Object.freeze({ kind: "number", value: previous.value + 1 });
}
...
function define(definitions, definition) {
  ...
  definitions.set(definition.id, { ...definition, state: "pending", value: undefined });
}
