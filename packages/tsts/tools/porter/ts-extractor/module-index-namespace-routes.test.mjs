...
`],
  ...
  assert.deepEqual(environments.get("pkg/use.ts").get("d"), { kind: "number", value: 5 });
  ...
  assert.deepEqual(environments.get("pkg/use.ts").get("family.N.K"), { kind: "number", value: 3 });
  assert.equal(environments.get("pkg/use.ts").get("family.N.hidden"), undefined);
  assert.deepEqual(environments.get("pkg/use.ts").get("family.N.Inner.Value"), { kind: "number", value: 6 });
  assert.equal(environments.get("pkg/use.ts").get("family.N.PrivateInner.Value"), undefined);
  assert.deepEqual(environments.get("pkg/use.ts").get("family.E.One"), { kind: "number", value: 1 });
  assert.deepEqual(environments.get("pkg/use.ts").get("family.E.Two"), { kind: "number", value: 2 });
  assert.deepEqual(environments.get("pkg/use.ts").get("family.Dotted.Inner.Value"), { kind: "number", value: 10 });
});
...
`],
  ...
  const environments = buildIndexedModuleValueEnvironments(counted.api, index);
  assert.deepEqual(environments.get("namespace/use.ts").get("ns.V"), { kind: "number", value: 11 });
  assert.deepEqual(environments.get("namespace/use.ts").get("defaultNamespace.V"), { kind: "number", value: 11 });
...
test("namespace routes stay independent and hide private qualified members", async (t) => {
  ...
  const environments = buildIndexedModuleValueEnvironments(counted.api, index);
  assert.deepEqual(environments.get("routes/use.ts").get("ns.V"), { kind: "number", value: 17 });
});
