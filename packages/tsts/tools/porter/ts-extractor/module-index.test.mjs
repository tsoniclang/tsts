...
test("export-star excludes default while explicit exports shadow stars", async (t) => {
  ...
  const environments = buildIndexedModuleValueEnvironments(counted.api, index);
  assert.deepEqual(environments.get("pkg/use-shadow.ts").get("V"), { kind: "number", value: 1 });
  assert.throws(
    () => indexTypeScriptModuleSources(counted.api, new Map([
...
test("star cycles allow exact exits, deduplicate one origin, and reject true ambiguity", async (t) => {
  ...
  const environments = buildIndexedModuleValueEnvironments(counted.api, index);
  assert.deepEqual(environments.get("cycle/use-a.ts").get("V"), { kind: "number", value: 7 });
  assert.deepEqual(environments.get("cycle/use-same.ts").get("V"), { kind: "number", value: 7 });
  assert.throws(
    () => indexTypeScriptModuleSources(counted.api, new Map([
      ["cycle/origin.ts", "export interface Exit {} export const V: number = 7;"],
...
`],
  ...
  const environments = buildIndexedModuleValueEnvironments(counted.api, index);
  assert.deepEqual(environments.get("constants/use.ts").get("good"), { kind: "number", value: 4 });
  ...
  assert.deepEqual(environments.get("constants/source.ts").get("invalid"), { kind: "number", value: NaN });
