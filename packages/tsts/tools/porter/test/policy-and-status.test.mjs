...
test("extractor snapshots reject schema drift, removed fields, unknown units, and count drift", () => {
  ...
  assert.ok(validatePorterSnapshot({ ...snapshot, files: [{ ...file, parseError: "broken" }] }, config).some((issue) => issue.includes("unknown snapshot-schema-13 key 'parseError'")));
  ...
  for (const key of ["generated", "imports", "metadata", "sourceHash", "units"]) {
    ...
    delete missing[key];
    assert.ok(
      validatePorterSnapshot({ ...snapshot, files: [missing] }, config).some((issue) => issue.includes(`missing required snapshot-schema-13 key '${key}'`)),
  ...
  for (const key of ["endOffset", "exported", "name", "parameters", "qualifiedName", "signature", "snippet", "startOffset", "typeParameterDetails"]) {
    ...
    delete missing[key];
    assert.ok(
      validatePorterSnapshot({ ...snapshot, files: [{ ...file, units: [missing] }] }, config).some((issue) => issue.includes(`missing required snapshot-schema-13 key '${key}'`)),
      `missing unit field ${key} must fail closed`,
    );
  }
  for (const key of ["nodeKindCounts", "featureCounts"]) {
    assert.ok(validatePorterSnapshot({ ...snapshot, files: [{ ...file, [key]: {} }] }, config).some((issue) => issue.includes(`unknown snapshot-schema-13 key '${key}'`)));
    assert.ok(validatePorterSnapshot({ ...snapshot, files: [{ ...file, units: [{ ...unit, [key]: {} }] }] }, config).some((issue) => issue.includes(`unknown snapshot-schema-13 key '${key}'`)));
  }
