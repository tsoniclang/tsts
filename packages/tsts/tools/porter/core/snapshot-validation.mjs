...
export function compareAllowedKeys(value, allowed, label, issues) {
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) issues.push(`${label} contains unknown snapshot-schema-5 key '${key}'`);
  }
}

export function requireKeys(value, required, label, issues) {
  for (const key of required) {
    if (!Object.hasOwn(value, key)) issues.push(`${label} is missing required snapshot-schema-5 key '${key}'`);
  }
}
...
export function validateImportContract(imported, label, issues) {
  ...
  const unresolved = imported?.packageName === undefined && imported?.path !== "C";
...
function validateUnitIdentityGroup(base, entries, issues) {
  ...
  for (const entry of entries) {
    ...
    if (entry.id === base) ordinal = 1;
      ...
      const match = new RegExp(`^${escapeRegExp(base)}::#([2-9][0-9]*)$`).exec(entry.id ?? "");
