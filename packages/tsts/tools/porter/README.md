...
- `npm run porter:facades -- --core-runtime --force` regenerates only `go/scalars.ts` and `go/compat.ts` from the same facade renderer. This is the bootstrap path when a core runtime change is required before the TypeScript parser can rebuild; it validates the pinned source but deliberately does not render or validate external facades. `--core-runtime --check` verifies that exact two-file subset.
- `npm run porter:value-operations` verifies the declaration-derived Go value-operation modules. Add `-- --write` to generate missing modules and `-- --write --force` to replace differing generated modules. This command first runs the complete signature audit, then consumes its exact TypeScript storage mappings and reviewed/generator-owned provider evidence. The generated-artifact audit is intentionally not a prerequisite of that signature audit: making derived output a prerequisite of the evidence that derives it would create a bootstrap cycle. `porter:status` records this post-signature audit as `not-run`; `porter:verify` runs it and requires it to be complete and clean.
