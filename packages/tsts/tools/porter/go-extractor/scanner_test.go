...
func TestDeclarationSignaturesExcludeCommentsWithoutLosingShape(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "comments.go")
	writeTestFile(t, path, `package sample
// Read documentation.
func Read(value int) int { return value } // trailing function comment
// Alias documentation.
type Alias = int // explanatory alias comment
`)
	report := scanGoFile(root, path, "example.test/sample")
	function := requireSyntaxUnit(t, report, "func", "Read")
	alias := requireSyntaxUnit(t, report, "type", "Alias")
	if function.Signature != "func Read(value int) int" {
		t.Fatalf("function signature retained comment trivia: %q", function.Signature)
	}
	if alias.Signature != "Alias = int" || alias.TypeExpression == nil || alias.TypeExpression.Text != "int" {
		t.Fatalf("type signature lost shape or retained comment trivia: %#v", alias)
	}
}
