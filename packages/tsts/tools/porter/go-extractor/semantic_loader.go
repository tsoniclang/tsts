...
func applyGoSemanticEvidence(root string, modulePath string, snapshot *Snapshot) {
	plan := semanticBuildProfiles(root, snapshot)
	...
	applySemanticImportNames(snapshot, resolvedImportNames)
