...
func TestDeclarationCorrelationUsesPhysicalOffsets(t *testing.T) {
	...
	blankCount := 0
	for _, unit := range file.Units {
		if unit.Kind != "func" && unit.Kind != "varGroup" {
			continue
		}
	...
	if initCount != 2 || blankCount != 2 {
		t.Fatalf("same-line declarations were not preserved: init=%d blanks=%d units=%#v", initCount, blankCount, file.Units)
	}
}
...
func declarationSnapshot(t *testing.T, root string, modulePath string) Snapshot {
	...
	applyGoSemanticEvidence(root, modulePath, &snapshot)
	return snapshot
}
