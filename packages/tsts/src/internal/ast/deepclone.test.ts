// Mirror of internal/ast/deepclone_test.go (TestDeepCloneNodeSanityCheck).
// Like the Go test, every input is parsed as /main.ts (the table's jsx flag is
// unused upstream), the file is deep-cloned, and the original/clone trees are
// compared level by level: distinct node identities, equal child counts.
import { test } from "node:test";
import assert from "node:assert/strict";
import type { GoPtr } from "../../go/compat.js";
import type { Node } from "./spine.js";
import { NewNodeFactory, Node_VisitEachChild } from "./spine.js";
import type { NodeFactoryHooks } from "./spine.js";
import { NodeFactory_DeepCloneNode } from "./deepclone.js";
import type { NodeVisitorHooks } from "./visitor.js";
import { NewNodeVisitor } from "./visitor.js";
...
interface NodeComparisonWorkItem {
  original: GoPtr<Node>;
  copy: GoPtr<Node>;
}

function getChildren(node: GoPtr<Node>): Array<GoPtr<Node>> {
  const children: Array<GoPtr<Node>> = [];
...
const data: Array<{ title: string; input: string; jsx?: boolean }> = [
...
for (const rec of data) {
  test(`Clone ${rec.title}`, () => {
    ...
    let work: NodeComparisonWorkItem[] = [{ original: file, copy: clone }];

    while (work.length > 0) {
      const nextWork: NodeComparisonWorkItem[] = [];
      for (const item of work) {
        assert.notStrictEqual(item.original, item.copy);
        const originalChildren = getChildren(item.original);
        const copyChildren = getChildren(item.copy);
        assert.equal(originalChildren.length, copyChildren.length);
        for (let i = 0; i < originalChildren.length; i++) {
          nextWork.push({ original: originalChildren[i]!, copy: copyChildren[i]! });
        }
      }
      work = nextWork;
    }
