 ...
 * @tsgo-override {"category":"runtime-representation","allow":["signature"],"reason":"The VFS Stat contract returns a nil FileInfo interface for missing paths and stat failures; callers pair that sentinel with filesystem-specific existence/error handling. TypeScript FileInfo | undefined preserves the interface-nil result across every FS implementation.","goSignature":"interface{AppendFile:(string,string)=>packages/tsts/src/go/compat.ts::GoError;Chtimes:(string,packages/tsts/src/go/time.ts::Time,packages/tsts/src/go/time.ts::Time)=>packages/tsts/src/go/compat.ts::GoError;DirectoryExists:(string)=>packages/tsts/src/go/scalars.ts::bool;FileExists:(string)=>packages/tsts/src/go/scalars.ts::bool;GetAccessibleEntries:(string)=>packages/tsts/src/internal/vfs/vfs.ts::Entries;ReadFile:(string)=>[string,packages/tsts/src/go/scalars.ts::bool];Realpath:(string)=>string;Remove:(string)=>packages/tsts/src/go/compat.ts::GoError;Stat:(string)=>packages/tsts/src/internal/vfs/vfs.ts::FileInfo;UseCaseSensitiveFileNames:()=>packages/tsts/src/go/scalars.ts::bool;WalkDir:(string,packages/tsts/src/internal/vfs/vfs.ts::WalkDirFunc)=>packages/tsts/src/go/compat.ts::GoError;WriteFile:(string,string)=>packages/tsts/src/go/compat.ts::GoError}","tsSignature":"interface{AppendFile:(string,string)=>packages/tsts/src/go/compat.ts::GoError;Chtimes:(string,packages/tsts/src/go/time.ts::Time,packages/tsts/src/go/time.ts::Time)=>packages/tsts/src/go/compat.ts::GoError;DirectoryExists:(string)=>packages/tsts/src/go/scalars.ts::bool;FileExists:(string)=>packages/tsts/src/go/scalars.ts::bool;GetAccessibleEntries:(string)=>packages/tsts/src/internal/vfs/vfs.ts::Entries;ReadFile:(string)=>[string,packages/tsts/src/go/scalars.ts::bool];Realpath:(string)=>string;Remove:(string)=>packages/tsts/src/go/compat.ts::GoError;Stat:(string)=>packages/tsts/src/go/compat.ts::GoPtr<packages/tsts/src/internal/vfs/vfs.ts::FileInfo>;UseCaseSensitiveFileNames:()=>packages/tsts/src/go/scalars.ts::bool;WalkDir:(string,packages/tsts/src/internal/vfs/vfs.ts::WalkDirFunc)=>packages/tsts/src/go/compat.ts::GoError;WriteFile:(string,string)=>packages/tsts/src/go/compat.ts::GoError}"}
 *
 * Go source:
 * FS interface {
 ...
 * @tsgo-override {"category":"runtime-representation","allow":["signature"],"reason":"Entries.Symlinks is explicitly nil when symlink information was not collected, which is observably different from a present empty map proving that no returned entry is a symlink. Consumers use that distinction to decide whether entries must be rechecked.","goSignature":"interface{Directories:packages/tsts/src/go/compat.ts::GoSlice<string>;Files:packages/tsts/src/go/compat.ts::GoSlice<string>;Symlinks:packages/tsts/src/go/compat.ts::GoMap<string,{__tsgoEmpty?:never}>}","tsSignature":"interface{Directories:packages/tsts/src/go/compat.ts::GoSlice<string>;Files:packages/tsts/src/go/compat.ts::GoSlice<string>;Symlinks:packages/tsts/src/go/compat.ts::GoPtr<packages/tsts/src/go/compat.ts::GoMap<string,{__tsgoEmpty?:never}>>}"}
 *
 * Go source:
 * Entries struct {
 * 	Files       []string
