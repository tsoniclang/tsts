...
export function Program_GetRedirectTargets(receiver: GoPtr<Program>, path: Path): GoSlice<string> {
  return receiver!.__tsgoEmbedded0!.redirectTargetsMap.get(path) ?? GoNilSlice();
}
...
export function Program_GetSourceFileFromReference(receiver: GoPtr<Program>, origin: GoPtr<SourceFile>, ref: GoPtr<FileReference>): GoPtr<SourceFile> {
  ...
  const supportedExtensionsBase = GetSupportedExtensions(Program_Options(receiver), GoNilSlice());
...
export function NewProgram(opts: ProgramOptions): GoPtr<Program> {
  ...
  const p: Program = {
    ...
    programDiagnostics: GoNilSlice(),
    hasEmitBlockingDiagnostics: { M: GoNilMap() },
    sourceFilesToEmitOnce: new Once(),
    sourceFilesToEmit: GoNilSlice(),
  ...
  if (resolvedModules !== undefined) {
    for (const [cacheKey, resolution_] of resolvedModules) {
      ...
      if ((!resolved || (!ResolvedModule_IsProviderVirtual(resolution) && !ExtensionIsOneOf(resolution!.Extension, SupportedTSExtensionsWithJsonFlat as GoSlice<string>))) &&
        ...
        unresolvedImports = GoSliceAppend(unresolvedImports, cacheKey.Name, GoStringValueOps);
      }
    }
  }
  return unresolvedImports;
  ...
  const walk: Visitor = (node) => {
    ...
    if (node!.Kind === KindParameter && HasDecorators(node)) {
      ...
      if (decorator !== undefined) {
        diags = GoSliceAppend(diags, NewDiagnostic(file, decorator!.Loc, diagnostics.Decorators_are_not_valid_here), GoPointerValueOps<Diagnostic>());
      }
    }
...
export function Program_verifyCompilerOptions(receiver: GoPtr<Program>): void {
  ...
  const createOptionDiagnosticInObjectLiteralSyntax = (objectLiteral: GoPtr<ObjectLiteralExpression>, onKey: bool, key1: string, key2: string, message: GoPtr<Message>, ...args: GoInterface<unknown>[]): GoPtr<Diagnostic> => {
    ...
    if (diag !== undefined) {
      receiver!.programDiagnostics = GoSliceAppend(receiver!.programDiagnostics, diag.v, GoPointerValueOps<Diagnostic>());
    }
    return diag?.v;
  };
  ...
  const createCompilerOptionsDiagnostic = (message: GoPtr<Message>, ...args: GoInterface<unknown>[]): GoPtr<Diagnostic> => {
    ...
    receiver!.programDiagnostics = GoSliceAppend(receiver!.programDiagnostics, diag, GoPointerValueOps<Diagnostic>());
    return diag;
  };
  ...
  const createRemovedOptionDiagnostic = (name: string, value: string, useInstead: string): void => {
    ...
    let args: GoSlice<GoInterface<unknown>> = GoNilSlice();
    if (value === "") {
      message = diagnostics.Option_0_has_been_removed_Please_remove_it_from_your_configuration;
      args = [name];
    } else {
  ...
  if (!Tristate_IsTrue(options!.NoEmit) && !Tristate_IsTrue(options!.SuppressOutputPathCheck)) {
    const emitFilesSeen: Set<string> = { M: GoNilMap() };
...
export function Program_getBindAndCheckDiagnosticsWithChecker(receiver: GoPtr<Program>, ctx: GoInterface<Context>, fileChecker: GoPtr<Checker>, sourceFile: GoPtr<SourceFile>): GoSlice<GoPtr<Diagnostic>> {
  ...
  recordBoundSourceFileExtensionFacts(receiver!.opts, sourceFile);
  let diags: GoSlice<GoPtr<Diagnostic>> = slices.Clip(SourceFile_BindDiagnostics(sourceFile));
  diags = GoSliceAppendSlice(diags, Checker_GetDiagnostics(fileChecker, ctx, sourceFile), GoPointerValueOps<Diagnostic>());
  ...
  if (isCheckJS) {
    diags = GoSliceAppendSlice(diags, SourceFile_JSDocDiagnostics(sourceFile), GoPointerValueOps<Diagnostic>());
  }
  ...
  for (const [, directive] of directivesByLine) {
    if (directive.Kind === CommentDirectiveKindExpectError) {
      result = GoSliceAppend(result, NewDiagnostic(sourceFile, directive.Loc, diagnostics.Unused_ts_expect_error_directive), GoPointerValueOps<Diagnostic>());
    }
  }
  return result;
}
...
export function Program_getDiagnosticsWithPrecedingDirectives(receiver: GoPtr<Program>, sourceFile: GoPtr<SourceFile>, diags: GoSlice<GoPtr<Diagnostic>>): [GoSlice<GoPtr<Diagnostic>>, GoMap<int, CommentDirective>] {
  if (sourceFile!.CommentDirectives.length === 0) {
    return [diags, GoNilMap()];
  }
  ...
  for (const diagnostic of diags) {
    ...
    if (!ignoreDiagnostic) {
      filtered = GoSliceAppend(filtered, diagnostic, GoPointerValueOps<Diagnostic>());
    }
  }
  return [filtered, directivesByLine];
}
...
export function Program_getSuggestionDiagnosticsWithChecker(receiver: GoPtr<Program>, ctx: GoInterface<Context>, fileChecker: GoPtr<Checker>, sourceFile: GoPtr<SourceFile>): GoSlice<GoPtr<Diagnostic>> {
  ...
  let diags: GoSlice<GoPtr<Diagnostic>> = slices.Clip(sourceFile!.BindSuggestionDiagnostics);
  diags = GoSliceAppendSlice(diags, Checker_GetSuggestionDiagnostics(fileChecker, ctx, sourceFile), GoPointerValueOps<Diagnostic>());
  return diags;
}
...
export function Program_Emit(receiver: GoPtr<Program>, ctx: GoInterface<Context>, options: EmitOptions): GoPtr<EmitResult> {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    emitters = GoSliceAppend(emitters, e, GoPointerValueOps<emitterType>());
    const [host, done] = newEmitHost(ctx, receiver, sourceFile);
...
export function CombineEmitResults(results: GoSlice<GoPtr<EmitResult>>): GoPtr<EmitResult> {
  ...
  for (const emitResult of results) {
    ...
    result.Diagnostics = GoSliceAppendSlice(result.Diagnostics, emitResult!.Diagnostics, GoPointerValueOps<Diagnostic>());
    result.EmittedFiles = GoSliceAppendSlice(result.EmittedFiles, emitResult!.EmittedFiles, GoStringValueOps);
    if (!GoSliceIsNil(emitResult!.SourceMaps)) {
      result.SourceMaps = GoSliceAppendSlice(result.SourceMaps, emitResult!.SourceMaps, GoPointerValueOps<SourceMapEmitResult>());
    }
  }
  return result;
}
...
export function GetDiagnosticsOfAnyProgram(ctx: GoInterface<Context>, program: GoInterface<ProgramLike>, file: GoPtr<SourceFile>, skipNoEmitCheckForDtsDiagnostics: bool, getBindDiagnostics: GoFunc<(arg0: GoInterface<Context>, arg1: GoPtr<SourceFile>) => GoSlice<GoPtr<Diagnostic>>>, getSemanticDiagnostics: GoFunc<(arg0: GoInterface<Context>, arg1: GoPtr<SourceFile>) => GoSlice<GoPtr<Diagnostic>>>): GoSlice<GoPtr<Diagnostic>> {
  ...
  allDiagnostics = GoSliceAppendSlice(allDiagnostics, program!.GetSyntacticDiagnostics(ctx, file), GoPointerValueOps<Diagnostic>());

  // If we didn't have any syntactic errors, then also try getting the program (options),
  // global and semantic errors.
  if (allDiagnostics.length === configFileParsingDiagnosticsLength) {
    allDiagnostics = GoSliceAppendSlice(allDiagnostics, program!.GetProgramDiagnostics(), GoPointerValueOps<Diagnostic>());

    getBindDiagnostics!(ctx, file);

    if (Tristate_IsFalseOrUnknown(program!.Options()!.ListFilesOnly)) {
      allDiagnostics = GoSliceAppendSlice(allDiagnostics, program!.GetGlobalDiagnostics(ctx), GoPointerValueOps<Diagnostic>());

      if (allDiagnostics.length === configFileParsingDiagnosticsLength) {
        allDiagnostics = GoSliceAppendSlice(allDiagnostics, getSemanticDiagnostics!(ctx, file), GoPointerValueOps<Diagnostic>());
        allDiagnostics = GoSliceAppendSlice(allDiagnostics, program!.GetGlobalDiagnostics(ctx), GoPointerValueOps<Diagnostic>());
      }
      ...
      if ((skipNoEmitCheckForDtsDiagnostics || Tristate_IsTrue(program!.Options()!.NoEmit)) && CompilerOptions_GetEmitDeclarations(program!.Options()) && allDiagnostics.length === configFileParsingDiagnosticsLength) {
        allDiagnostics = GoSliceAppendSlice(allDiagnostics, program!.GetDeclarationDiagnostics(ctx, file), GoPointerValueOps<Diagnostic>());
      }
    }
  }
  return allDiagnostics;
...
export function Program_GetSymlinkCache(receiver: GoPtr<Program>): GoPtr<KnownSymlinks> {
  return lazyValue_getValue(receiver!.knownSymlinks, () => {
    ...
    const seenPackageJsons: Set<Path> = { M: GoNilMap() };
