...
import { GoInterfaceValueOps, GoSliceStore } from "../../go/compat.js";
...
export function wrapTypes(types: GoSlice<GoPtr<Type>>): GoSlice<GoInterface<TracedType>> {
  ...
  for (let i = 0; i < types.length; i++) {
    GoSliceStore(result, i, wrapType(GoSliceLoad(types, i, GoPointerValueOps<Type>())), GoInterfaceValueOps<TracedType>());
  }
  return result;
}
