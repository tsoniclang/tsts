...
export function NodeBuilderImpl_tryGetResolvedSymbolFromTypeNode(receiver: GoPtr<NodeBuilderImpl>, node: GoPtr<Node>): GoPtr<Symbol> {
  ...
  const links = LinkStore_TryGet(receiver!.ch!.symbolNodeLinks, node);
  if (links === undefined) {
    return undefined;
  }
  return links!.resolvedSymbol;
  ...
  if (sym!.Declarations !== undefined && sym!.Declarations.length > 0) {
    ...
    if (name !== undefined) {
      if (IsComputedPropertyName(name) && (sym!.CheckFlags & CheckFlagsLate) === 0) {
        if (LinkStore_Has(receiver!.ch!.valueSymbolLinks, sym) && LinkStore_Get(receiver!.ch!.valueSymbolLinks, sym)!.nameType !== undefined && (LinkStore_Get(receiver!.ch!.valueSymbolLinks, sym)!.nameType!.flags & TypeFlagsStringOrNumberLiteral) !== 0) {
