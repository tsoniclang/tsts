...
export interface WatchManager {
  ...
  doCyclePending: bool;
  runLoop: watchRunLoop | undefined;
...
export function WatchManager_DoCycleCh(receiver: GoPtr<WatchManager>): GoChan<{ readonly __tsgoEmpty?: never }, "receive"> {
  return receiver!.doCycleCh as unknown as GoChan<{ readonly __tsgoEmpty?: never }, "receive">;
}
...
export function WatchManager_signalDoCycle(receiver: GoPtr<WatchManager>): void {
  receiver!.doCyclePending = true as bool;
  WatchManager_scheduleCycle(receiver);
}

function WatchManager_scheduleCycle(receiver: GoPtr<WatchManager>): void {
  const runLoop = receiver!.runLoop;
  if (runLoop === undefined || runLoop.stopped || runLoop.scheduled || !receiver!.doCyclePending) {
    return;
  }
  runLoop.scheduled = true as bool;
  queueMicrotask((): void => {
    if (receiver!.runLoop !== runLoop || runLoop.stopped) {
      return;
    }
    runLoop.scheduled = false as bool;
    if (runLoop.ctx.Err() !== undefined) {
      WatchManager_stopRunLoop(receiver, runLoop);
      return;
    }
    if (!receiver!.doCyclePending) {
      return;
    }
    receiver!.doCyclePending = false as bool;
    runLoop.doCycle();
    if (runLoop.ctx.Err() !== undefined) {
      WatchManager_stopRunLoop(receiver, runLoop);
      return;
...
function WatchManager_stopRunLoop(receiver: GoPtr<WatchManager>, runLoop: watchRunLoop): void {
  if (receiver!.runLoop !== runLoop || runLoop.stopped) {
    return;
  }
  runLoop.stopped = true as bool;
  receiver!.runLoop = undefined;
...
export function WatchManager_RunLoop(receiver: GoPtr<WatchManager>, ctx: Context, doCycle: () => void): void {
  if (receiver!.runLoop !== undefined) {
    throw new globalThis.Error("watch manager run loop already started");
  }
  const runLoop: watchRunLoop = {
    ctx,
    doCycle,
    scheduled: false as bool,
    stopped: false as bool,
  };
  receiver!.runLoop = runLoop;
