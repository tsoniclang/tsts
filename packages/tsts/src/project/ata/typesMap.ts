...
const safeFileNameToTypeNameEntries: readonly (readonly [string, string])[] = [
...
export const safeFileNameToTypeName: ReadonlyMap<string, string> = new Map(safeFileNameToTypeNameEntries);

export class TypesMap {
  private readonly packageToTypings = new Map<string, string>();

  constructor(entries: Iterable<TypesMapEntry> = safeFileNameToTypeNameEntries.map(
    ([packageName, typingsPackageName]) => ({ packageName, typingsPackageName }),
  )) {
...
export function getTypingPackageNameFromSafeFileName(fileName: string): string | undefined {
  return safeFileNameToTypeName.get(fileName);
}

export function newTypesMap(entries?: Iterable<TypesMapEntry>): TypesMap {
  return entries === undefined ? new TypesMap() : new TypesMap(entries);
}
