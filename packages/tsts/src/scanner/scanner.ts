...
export function scanAll(text: string, options?: ScannerOptions): readonly ScannedToken[] {
  const scanner = createLiveScanner(
    text,
    options?.skipTrivia === undefined ? undefined : { skipTrivia: options.skipTrivia },
  );
  const tokens: ScannedToken[] = [];
  for (;;) {
    const kind = scanner.scan();
    tokens.push({ kind, pos: scanner.getTokenStart(), end: scanner.getTokenEnd(), text: scanner.getTokenText() });
...
// ---------------------------------------------------------------------------
// LiveScanner — THE scanner. Faithful, parser-decoupled, stateful token
// scanner. This is a faithful port of TS-Go's stateful scanner
...
// Mark/Rewind/ResetPos + the reScan*/scanJsx* re-tokenization families.
// Speculation lives in the parser, not here. `scanAll` above is a thin
// collector over this scanner (single Scan() loop, no separate rules).
//
