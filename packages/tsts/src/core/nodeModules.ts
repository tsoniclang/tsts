...
export function nodeCoreModules(): ReadonlySet<string> {
  if (_nodeCoreModulesCache === undefined) {
    _nodeCoreModulesCache = new Set(nodeCoreModuleEntries());
  }
  return _nodeCoreModulesCache;
}

export function nodeCoreModuleEntries(): readonly string[] {
...
export function isNodeCoreModule(moduleName: string): boolean {
  return nodeCoreModules().has(moduleName);
}

export function nodeCoreModuleNames(): readonly string[] {
  return Array.from(nodeCoreModules()).sort();
}

export function nonRelativeModuleNameForTypingCache(moduleName: string): string {
  if (isNodeCoreModule(moduleName)) return "node";
  return moduleName;
}
