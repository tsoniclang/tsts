...
import {
  ...
  selectedTsonicCoreCertificationModules,
  tsonicCoreCertificationPath,
} from "../scripts/tsonic-core-certification.mjs";

const modules = await selectedTsonicCoreCertificationModules();
...
test("source-core certification is an exact deterministic provider projection", async () => {
  ...
  const expected = buildTsonicCoreCertificationSource(modules);
  assert.equal(actual, expected);
  assert.equal([...declarationsByModule.values()].reduce((sum, entries) => sum + entries.length, 0), 79);
});
    ...
    [
      {
        ...
        declarations: 27,
        names: [
          "DataLayout", "FixedArray", "FunctionPointer", "MemoryFieldBinding", "MemoryFieldLayout", "MemoryLayout",
      ...
      },
      {
        ...
        names: [
          ...
          "addressIntegerToRawPointer", "alignOf", "allocatePointer", "attribute", "bindMemoryField", "bindMemoryRecord", "bindPointer",
          ...
          "sizeOf", "strideOf", "toRawPointer", "unroll", "viewPointer",
        ].sort(),
      },
      {
...
test("certification carries exact address domains and child-layout arguments", () => {
  const source = buildTsonicCoreCertificationSource(modules);
