...
const canonicalAcceptance = Object.freeze({
...
test("target profile has one stable semantic identity", async () => {
  ...
  assert.deepEqual(left.acceptance, canonicalAcceptance);
...
test("target profile rejects invalid optimization acceptance", async () => {
  ...
  for (const acceptance of [
    {},
    { ...canonicalAcceptance, pointerKeyMapCount: 0 },
    { ...canonicalAcceptance, staticPropertyLocationCount: 0 },
    { ...canonicalAcceptance, staticPropertyLocationClassCount: 0 },
    { ...canonicalAcceptance, fallback: true },
  ]) {
    await writeFile(path, JSON.stringify({
      ...canonicalProfile(),
      acceptance,
